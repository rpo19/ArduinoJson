...
class JsonDeserializer {
 ...
 private:
  ...
  DeserializationError parseNumericValue(JsonVariantData &result) {
    ...
    if (isInteger(memoryPool)) {
      result.setInteger(parseInteger<JsonInteger>(memoryPool));
    } else if (isFloat(memoryPool)) {
      result.setFloat(parseFloat<JsonFloat>(memoryPool));
    } else if (!strcmp(memoryPool, "true")) {
      result.setBoolean(true);
    } else if (!strcmp(memoryPool, "false")) {
      result.setBoolean(false);
    } else if (!strcmp(memoryPool, "null")) {
      result.setNull();
    } else {
      return DeserializationError::InvalidInput;
    }
